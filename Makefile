...
cover:
	...
	GOCOVERDIR=$(CURDIR)/$(GOCOVERDIR)/integration go test -v -count=1 -parallel=1 main_test.go
	go test -v -cover -parallel=1 ./... -args -test.gocoverdir=$(CURDIR)/$(GOCOVERDIR)/unit
